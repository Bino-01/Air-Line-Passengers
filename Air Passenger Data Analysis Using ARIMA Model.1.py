...
data = pd.read_csv('Air Passenger.csv')
(printdata.head(7))
...
data = pd.read_csv('Air Passenger.csv',parse_dates=['Date'],index_col='Date',date_parser=dateparse)
print('\n Parsed Data:')
print(data.head(7))
...
data=pd.read_csv('Air Passenger.csv', index_col=0, parse_dates=True)
...
plt.legend(loc='best')
plt.title('Rolling Mean & Standard Deviation - Air Passenger 2007')
...
# Test Statistic > Critical Values. Therefore the null hypothesis is not rejected and the Air Passenger 2007 TS is Stationary.
...
ts_log = np.log(ts)
plt.plot(ts_log)
plt.title('Plot Log TS - Air Passenger 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title('Plot Log TS & Moving Average - Air Passenger 2007', Fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title ('Plot Log Time Series & Exponential Weighted -Air Passenger 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
plt.ylabel('price', fontsize = 15)	
...
plt.title ('Plot Log Diff TS -Air Passenger 2007', fontsize = 15)
...
plt.title ('Original Air Passenger 2007', fontsize = 15)
plt.legend(loc='best')
plt.subplot(412)
plt.plot(trend, label='Trend')
plt.legend(loc='best')
plt.title ('Trend Air Passenger 2007', fontsize = 15)
plt.subplot(413)
plt.plot(seasonal,label='Seasonality')
plt.legend(loc='best')
plt.title ('Seasonaity Air Passenger', fontsize = 15)
plt.subplot(414)
plt.plot(residual, label='Residuals')
plt.legend(loc='best')
plt.title ('Residual Air Passenger 2007', fontsize = 15)
plt.tight_layout()
...
plt.title('Air Passenger - RMSE: %.4f'% np.sqrt(sum((predictions_ARIMA-ts)**2)/len(ts)))
